  ...
  return (
    <div className="flex mb-1 sm:mb-2">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 ${star <= rating ? 'text-yellow-400' : 'text-gray-300'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
  ...
  return (
    <div className="relative border border-[#084710] rounded-xl bg-white px-3 py-2 sm:px-3 sm:py-2 flex flex-col h-full max-w-[320px] sm:max-w-none mx-4 sm:mx-auto">
      ...
      <button
        ...
        className={`absolute top-4 right-4 bg-white hover:bg-gray-50 p-2 rounded-full shadow-md transition-all duration-300 group z-10
